...
def get_args():
    ...
    parser.add_argument('--k', type=int, default=5, help='Knn')
    parser.add_argument('--average_entropy', type=bool, default=False, help='Only use kth element or use average of k elements')
    parser.add_argument('--embedding_dim', type=int, default=150, help='Embedding dim')
    parser.add_argument('--capacity', type=int, default=int(1e5), help='Embedding Replay Buffer size')
    parser.add_argument('--norm_int_type', type=str, default="min_max", help='Use rms, min max scale to normalize intrinsic reward or do not normalize. "no": do not normalize. "rms": use running mean std. "min_max": use min max scale.')
...
def train(config):
    ...
    random_encoder_model = Random_Encoder_Model(config.embedding_dim, config.use_layer_init)
    replay_buffer = Embedding_Replay_Buffer(config.capacity,
    ...
    agent = Agent(envs = envs, world = config.world, stage = config.stage, action_type = config.action_type, num_envs = config.num_envs,
              ...
              model = model, random_encoder_model = random_encoder_model, gamma = config.gamma, gamma_int = config.gamma_int,
              ...
              int_adv_coef = config.int_adv_coef, embedding_dim = config.embedding_dim, k = config.k, 
              average_entropy = config.average_entropy, capacity = config.capacity, replay_buffer = replay_buffer, 
              norm_int_type = config.norm_int_type, device = "cuda:0" if torch.cuda.is_available() else "cpu")
    agent.train()
